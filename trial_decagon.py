...
def construct_placeholders(edge_types):
    ...
    placeholders.update({
        'feat_%d' % i: tf.placeholder(tf.float32, shape=())
...
mb_file = 'data/data_structures/MINIBATCH/MINIBATCH_RED_'+words[2]+d_text+\
...
with open(mb_file, 'rb') as f:
    ...
    for key in MB.keys():
        ...
        print(key,"Imported successfully")
for en in feat:
    coord,val,shape = feat[en]
    result = np.zeros(shape)
    for i in range(len(coord)):
        result[coord[i][0],coord[i][1]] = val[i]
    feat[en] = result
minibatch.feat = feat
...
output_data={}
out_file = 'results_training/TRAIN_DENSE'+words[2]+d_text+DSE*('_DSE_'+str(n_se_mono))+BDM*('_BDM')\
